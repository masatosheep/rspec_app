...
RSpec.describe FoodEnquete, type: :model do
  describe '正常系の機能' do
    context '回答する' do
      it '正しく登録できること 料理:やきそば food_id: 2,
        ...
        expect(answered_enquete.mail).to eq('tanaka@example.com')
  ...
  describe 'アンケート回答時の条件' do
    ...
    context '年齢を確認すること' do
      ...
      enqute_sato = FactoryBot.build(:food_enquete_sato)

        expect(enqute_sato).not_to be_valid
        # 星人のみ選択できる旨のメッセージが含まれること
        expect(enqute_sato.errors[:present_id]).to include(I18n.t('activerecord.errors.models.food_enquete.attributes.present_id.cannot_present_to_minor'))
      end

      it '成人はビール飲み放題を選択でること' do
        enqute_sato = FactoryBot.build(:food_enquete_sato, age: 20)

        expect(enqute_sato).to be_valid
      end
    end
  end
